...
The first judge evaluates each Question-Answer pair along six distinct dimensions, providing a judgement on how well each criterion is satisfied on a three-point ordinal scale.
\begin{enumerate}
      ...
      A question only satisfies this criterion if neither paper alone provides sufficient evidence to derive the complete answer.
      ...
      \item  \textbf{Evidence Distribution} examines whether both papers contribute distinct and necessary evidence, ensuring that required evidence is spread across both papers and that questions cannot be answered by using a single dominant source with redundant information.
      ...
      \item  \textbf{Decontextualization} validates that the question is self-contained and unambiguous in an open-domain setting. Thus, questions cannot contain context-specific references and must instead introduce relevant concepts, methods or entities by themselves in order to satisfy this criterion.
\end{enumerate}
